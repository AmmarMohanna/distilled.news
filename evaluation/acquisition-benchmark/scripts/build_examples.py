...
<item><guid>story-2</guid><title>افتتاح المكتبة</title><description>المكتبة مفتوحة للجميع.</description><link>https://example.com/library-ar</link><pubDate>Tue, 01 Sep 2026 10:00:00 GMT</pubDate></item>
    ...
    write("fixtures/rss.gold.json", {"complete_window": True, "window": window, "items": [
    ...
    write("fixtures/telegram.gold.json", {"complete_window": True, "window": window, "items": [telegram]})
    ...
    write("fixtures/x.gold.json", {"complete_window": True, "window": window, "items": [{"id": "12345", "text": "Library news from the test account", "published_at": "2026-09-01T09:00:00Z"}]})
    ...
    write("fixtures/linkedin.gold.json", {"complete_window": True, "window": window, "items": [{"id": linkedin["id"], "text": linkedin["text"], "published_at": linkedin["postedAt"]}]})
    ...
    routes = [route("direct", "direct_http"), route("zyte", "zyte_http"), route("unlocker", "brightdata_unlocker", zone=""),
              route("browser", "browser_playwright", network="isolated"),
              route("browser_standard", "browser_playwright", network="standard", host_egress_firewall_confirmed=False),
              route("rss_baseline", "rss", parser="baseline"), route("rss_feedparser", "rss", parser="feedparser"),
    ...
    for target in targets:
        if target["kind"] != "article": target.setdefault("options", {}).update(window)
    base = {"version": 1, "mode": "live", "data_dir": "../data/live", "repetitions": 2, "seed": 20260913,
            "budget": {"total_usd": 0, "providers": {}}, "costs": {"server_monthly_usd": None}, "routes": routes, "targets": targets,
            "chains": [{"id": "direct_then_managed_then_browser", "routes": ["direct", "zyte", "unlocker", "browser_standard"], "deadline_seconds": 90}],
            "notes": "Disabled templates: replace inputs, provide independent gold, set costs.server_monthly_usd, confirm provider schemas and caps before enabling routes. "
                     "browser (isolated) and browser_standard are separately scored variants; browser_standard needs a confirmed host egress firewall."}
    ...
    soak.update(repetitions=1, stage="soak", schedule={"rounds": 8, "interval_seconds": 86400, "rolling_window_hours": 24, "references_dir": "../data/live/references"},
                notes="Eight daily rounds span seven days. Rolling windows change every round, so source references go in references_dir/round-NNN/<target_id>.json or are attached to each child run. Add targets_by_round or daily_targets_dir entries for newly discovered URLs; original article targets repeat unchanged.")
